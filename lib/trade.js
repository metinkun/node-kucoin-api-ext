...
Trade.listFills = async function (params = {}) {
  ...
  do {
    ...
    params.pageSize = 500;
    let url = this.baseURL + endpoint + this.formatQuery(params);
    result = await axios.get(url, this.sign(endpoint, params, 'GET'));
    if (result.data.data.currentPage < result.data.data.totalPage) next = true;
    fills.push(...result.data.data.items);
  } while (next);
  result.data.data.items = fills;
  const totalSize = fills.reduce((p, c) => p + parseFloat(c.size), 0);
  const averagePrice = fills.reduce((p, c) => p + parseFloat(c.price) * parseFloat(c.size), 0) / totalSize;
  return {
    fills,
    averagePrice,
  };
