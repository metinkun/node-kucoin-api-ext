...
const Kucoin = {
  ...
  sign: function (endpoint, params, method) {
    ...
    let signatureResult = crypto.createHmac('sha256', this.secretKey).update(strForSign).digest('base64');
    header.headers['KC-API-SIGN'] = signatureResult;
    header.headers['KC-API-TIMESTAMP'] = nonce;
    header.headers['KC-API-KEY'] = this.apiKey;
    header.headers['KC-API-PASSPHRASE'] = crypto.createHmac('sha256', this.secretKey).update(this.passphrase).digest('base64');
