...
Sockets.initSocket = async function (params, eventHandler) {
  try {
    ...
    let websocket,
      ...
      connected = false;
    const connect = () => {
      await getSocketEndpoint(type, this.baseURL, this.environment, sign);
      ws = new WebSocket(websocket);
      Sockets.ws[topic] = ws;
    };
    await connect();
    setInterval(() => {
      if (!Sockets.ws[topic].connected) {
        await connect();
      }
    }, 10000);
    ...
    ws.on('open', () => {
      ...
      Sockets.ws[topic].connected = true;
    ...
    ws.on('close', () => {
      Sockets.ws[topic].connected = false;
